...
$(document).ready(function () {
	...
	$('#data_form').submit(function(e) {
		...
		var action_url = "" + base_url + "/saveOpini";
		var action_type = "Tambah";
		if (method === "PUT") {
			action_url = "" + base_url + "/updateOpini/" + $('#id').val();
			action_type = "Ubah";
		}
	...
	function loadData() {
        ...
        data_table  = $('#datatable').DataTable({            
            ...
            ajax: {
                "url": "" + base_url + '/getDataJson/opini',
                'type': 'GET',
                'dataType': 'JSON',
            ...
            },
			{
                ...
                render: function (data, type, row) {					
					...
					if (row.opini_publish === "y") {   
						return '<button onclick="updateStatus('+row.opini_id+","+"'n'"+","+"'/updateStatusOpini'"+')" class="btn btn-success btn-sm btn-akses"> <i class="fa fa-eye"></i> Publish</button>&nbsp;';
						//return '<input type="checkbox" onclick="updateStatus('+row.opini_id+","+"'n'"+","+"'/updateStatusOpini'"+')" checked data-toggle="toggle"><span class="label label-success"> Publish</span>';
                    } else {                                    
						return '<button onclick="updateStatus('+row.opini_id+","+"'y'"+","+"'/updateStatusOpini'"+')" class="btn btn-danger btn-sm btn-akses"> <i class="fa fa-eye-slash"></i> Tidak Publish</button>&nbsp;';
						//return '<input type="checkbox" onclick="updateStatus('+row.opini_id+","+"'y'"+","+"'/updateStatusOpini'"+')" data-toggle="toggle"><span class="label label-danger">Tidak Publish</span>';
                    }
                }
            }, {
                title: "Gambar",                
                ...
                render: function (data, type, row) {                    
					var result = '';
					if (row.opini_path === "" || row.opini_path === null || row.opini_path === "null") {						
						...
						result += '<a href="'+base_url+'/'+row.opini_path+row.opini_gambar_name+'" data-lightbox="gallery-group-1">';
						result += '<img src="'+base_url+'/'+row.opini_path+row.opini_gambar_name+'" style="width:200px; height:120px"/></a>';
						result += '</div>';
					}                 
                    return result;
                }
            ...
            "drawCallback": function (settings) {
                $('.btn-edit').on('click', function () {                    
					...
					location.href = "/editOpini/"+btoa(data.opini_id);	                                       
                });
                ...
                $('.btn-delete').on('click', function () {
                    ...
                    Lobibox.confirm({
                        iconClass: true,
                        title: 'Delete Data',                        
                        msg: 'Yakin Hapus Data "' + data.opini_judul + '"?',
                        callback: function ($this, type, ev) {
                            if(type=='yes'){
								var route = "/delete/opini/";
                                deleteProses(data.opini_id,route);
                            }        
                        }
                    });
                });
    ...
    var validator = $('#data_form').validate({
        ...
        rules: {
            opini_judul: { required: true },
			opini_publish: {required: true},			
            datepicker: {required: true},						
			opini_isi: {required: true},						
        },
