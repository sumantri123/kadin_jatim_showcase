...
$('#data_form').submit(function(e) {
	...
	if(cekfile===""){
		...
		var action_url = "" + base_url + "/saveRilisImage";
		var action_type = "Tambah";
		if (method === "PUT") {
			action_url = "" + base_url + "/updateRilisImage/" + $('#id').val();
			action_type = "Ubah";
		}
...
function loadData() {
	...
	data_table  = $('#datatable').DataTable({            
		...
		ajax: {
			"url": "" + base_url + '/getDataJson/rilis_image/'+id,
			'type': 'GET',
			'dataType': 'JSON',
		...
		columns: [
		{
			...
			title: "Nama File",
			data: "rilis_image_ori",
			width: "50%",
			visible: true,
			sortable: true,                
			render: function (data, type, row) {
				return row.rilis_image_ori;                    
			}
		},{
			title: "Gambar",                
			visible: true,
			...
			render: function (data, type, row) { 				
				var result = '';
				if (row.rilis_image_path === "" || row.rilis_image_path === null || row.rilis_image_path === "null") {						
					...
					result += '<a href="'+base_url+'/'+row.rilis_image_path+row.rilis_image_name+'" data-lightbox="gallery-group-1">';
					result += '<img src="'+base_url+'/'+row.rilis_image_path+row.rilis_image_name+'" style="width:200px; height:120px"/></a>';
					result += '</div>';
				}
				return result;
			}
			...
			render: function (data, type, row) {										                    
				...
				if (row.rilis_image_publish === "y") {
					return '<button onclick="updateStatus('+row.rilis_image_id+","+"'n'"+","+"'/updateStatusRilisImage'"+')" class="btn btn-success btn-sm btn-akses"> <i class="fa fa-eye"></i> Publish</button>&nbsp;';
				} else {       
					return '<button onclick="updateStatus('+row.rilis_image_id+","+"'y'"+","+"'/updateStatusRilisImage'"+')" class="btn btn-danger btn-sm btn-akses"> <i class="fa fa-eye-slash"></i> Tidak Publish</button>&nbsp;';						
				}
			}
		}],
			...
			$('.btn-delete').on('click', function () {				
				...
				Lobibox.confirm({
					...
					callback: function ($this, type, ev) {
						if(type=='yes'){
							var route = "/delete/rilis_image/";
							deleteProses(data.rilis_image_id,route);
						}        
					}
				});
			});
