				...
				<div id="transContent" class="panel panel-inverse" data-sortable-id="form-plugins-4">
					...
					<form class="form-horizontal form-bordered" id="data_form" method="post" enctype="multipart/form-data">
						<div class="panel-body panel-form">						
                            ...
                            <input type="hidden" class="form-control" id="id" value="{{($data['act']=='edit') ? encrypt($opini[0]->opini_id) : ''}}" name="id">
							<input type="hidden" class="form-control" id="act" value="{{$data['act']}}" name="act">
							<div class="form-group row">
								<label class="col-md-2 col-form-label">Judul Opini</label>
								<div class="col-md-10">
									<input type="text" class="form-control" id="opini_judul" name="opini_judul" placeholder="Judul Opini" value="{{($data['act']=='edit') ? $opini[0]->opini_judul : ''}}"/>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-2 col-form-label">Tanggal Opini</label>
								<div class="col-md-10">
									<div class="input-group date" id="datepicker-default" data-date-format="dd-mm-yyyy" value="04/1/2022">
										<input type="text" class="form-control" name="opini_tanggal" id="datepicker" placeholder="Tanggal Opini" value="{{($data['act']=='edit') ? $opini[0]->opini_tanggal : ''}}"/>
										<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
									</div>									
								</div>
							</div>
							<div class="form-group row">
								<label class="col-md-2 col-form-label">Isi Opini</label>
								<div class="col-md-10">
									<textarea class="summernote" id="opini_isi" name="opini_isi">{{($data['act']=='edit') ? $opini[0]->opini_isi : ''}}</textarea>
								</div>
							</div>
							@if ($data['act']=="edit")
							<div class="form-group row">
								...
								<div class="col-md-10">
									<div class="input-group">
										<select class="form-control selectpicker" id="opini_publish" name="opini_publish" data-size="10" data-live-search="true" data-style="btn-success">
                                            <option value="" selected>Apakah di Publish ?</option>
											<option value="y" {{($data['act']=='edit') ? $opini[0]->opini_publish == "y"  ? "selected" : "" : ""}}>Publish</option>
											<option value="t" {{($data['act']=='edit') ? $opini[0]->opini_publish == "t"  ? "selected" : "" : ""}}>Tidak Publish</option>                                            
                                        </select>
									</div>
								</div>
							</div>
								...
								<div class="col-md-10">
									...
									<div class="input-group" id="edit_upload">
										<div id="gallery" class="gallery" style="width:300px; height:220px;" />
											...
											@else					
												<a href="{{asset($opini[0]->opini_path.$opini[0]->opini_gambar_name) }}" data-lightbox="gallery-group-1">
												<img src="{{asset($opini[0]->opini_path.$opini[0]->opini_gambar_name) }}" style="width:300px; height:220px;"/></a>
											@endif
										</div>&emsp;
...
@push('scripts')
	<script src="{{ asset('additional/js/opini.js') }}"></script>
@endpush
