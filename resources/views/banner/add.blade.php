				...
				<div id="transContent" class="panel panel-inverse" data-sortable-id="form-plugins-4">
					...
					<form class="form-horizontal form-bordered" id="data_form" method="post" enctype="multipart/form-data">
						<div class="panel-body panel-form">						
                            ...
                            <input type="hidden" class="form-control" id="id" value="{{($data['act']=='edit') ? encrypt($banner[0]->banner_id) : ''}}" name="id">
							<input type="hidden" class="form-control" id="act" value="{{$data['act']}}" name="act">							
							<div class="form-group row">
								<label class="col-md-2 col-form-label">Deskripsi</label>
								<div class="col-md-10">
									<textarea class="form-control" id="deskripsi" name="deskripsi">{{($data['act']=='edit') ? $banner[0]->banner_deskripsi : ''}}</textarea>
								</div>
							</div>
							@if ($data['act']=="edit")
							<div class="form-group row">
								...
								<div class="col-md-10">
									<div class="input-group">
										<select class="form-control selectpicker" id="banner_publish" name="banner_publish" data-size="10" data-live-search="true" data-style="btn-success">
                                            <option value="" selected>Apakah di Publish ?</option>
											<option value="y" {{($data['act']=='edit') ? $banner[0]->banner_publish == "y"  ? "selected" : "" : ""}}>Publish</option>
											<option value="t" {{($data['act']=='edit') ? $banner[0]->banner_publish == "t"  ? "selected" : "" : ""}}>Tidak Publish</option>
                                        </select>
									</div>
								</div>
							</div>
								...
								<div class="col-md-10">
									...
									<div class="input-group" id="edit_upload">
										<div id="gallery" class="gallery" style="width:300px; height:220px;" />
											@if ($banner[0]->banner_path==null || $banner[0]->banner_path=="")
											...
											@else					
												<a href="{{asset($banner[0]->banner_path.$banner[0]->banner_name) }}" data-lightbox="gallery-group-1">
												<img src="{{asset($banner[0]->banner_path.$banner[0]->banner_name) }}" style="width:300px; height:220px;"/></a>
											@endif
										</div>&emsp;
...
@push('scripts')
	<script src="{{ asset('additional/js/banner.js') }}"></script>	
@endpush
